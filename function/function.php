...
if($id==5){ 
return '<span class="ml-3 tx-16 my-auto">
...
function getLastLoginFun($id){  
$rsaa=GetPageRecord('cLogin','userLogs','userid="'.$id.'"'); 
while($userss=mysqli_fetch_array($rsaa)){  
return $userss['cLogin'];
}
...
$selectaa='name';   
$whereaa='id="'.$id.'"'; 
$rsaa=GetPageRecord($selectaa,'roleMaster',$whereaa); 
while($result=mysqli_fetch_array($rsaa)){  
...
if($id!=''){ 
$selectaa='profilePhoto';   
$whereaa='id="'.$id.'"'; 
$rsaa=GetPageRecord($selectaa,'sys_userMaster',$whereaa); 
while($result=mysqli_fetch_array($rsaa)){
if( $result['profilePhoto']!=''){ 
return 'upload/'.stripslashes($result['profilePhoto']);
} else  {
return 'images/nouserimg.png';
}
}
}
}
...
if($id!=''){  
$selectaa='city,country';   
$whereaa='id="'.$id.'"'; 
$rsaa=GetPageRecord($selectaa,'destinationMaster',$whereaa); 
while($userss=mysqli_fetch_array($rsaa)){  
return $userss['city'].', '.$userss['country'];
}
}
}
